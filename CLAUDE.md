...
Mỗi scene là một "chapter" (30-120s) chứa nhiều **beats** — đơn vị sync giữa lời và hình.
  ...
  "scenes": [
    {
      "id": "s01",
      "order": 1,
      "narration": "Hãy hình dung một tín hiệu. Nó thực ra là tổng của các vector quay. Khi ta cộng chúng lại, ta được sóng vuông.",
      ...
      "beats": [                   // intra-scene sync points (Stage 2 sets these)
        {
          "id": "s01_b01",
          "order": 1,
          "trigger_phrase": "hình dung một tín hiệu",
          "visual_action": "Create signal waveform on axes",
          "narration_segment": "Hãy hình dung một tín hiệu.",
          "start_sec": null,       // điền bởi beat_timing resolver (sau stage 4)
          "duration_sec": null
        },
        {
          "id": "s01_b02",
          "order": 2,
          "trigger_phrase": "tổng của các vector quay",
          "visual_action": "Transform signal into rotating phasors",
          "narration_segment": "Nó thực ra là tổng của các vector quay.",
          "start_sec": null,
          "duration_sec": null
        },
        {
          "id": "s01_b03",
          "order": 3,
          "trigger_phrase": "cộng chúng lại",
          "visual_action": "Animate phasor sum converging to square wave",
          "narration_segment": "Khi ta cộng chúng lại, ta được sóng vuông.",
          "start_sec": null,
          "duration_sec": null
        }
      ],
      "manim_code": null,          // điền ở stage 3 (ONE continuous scene for all beats)
      ...
      "word_timestamps": null,     // điền ở stage 4 (cho phụ đề sync + beat timing)
...
Quy tắc:
- Stage chỉ được ghi field của mình.
- `duration_sec` chỉ được set ở stage 4 (voiceover) và là nguồn chân lý cho timeline ở stage 5.
- `beats[].start_sec` và `beats[].duration_sec` được tính bởi beat_timing resolver (chạy giữa stage 4 và 5), dựa trên word_timestamps + trigger_phrase matching.
- Manim codegen sinh **MỘT Scene liên tục** cho tất cả beats trong một scene — objects persist across beats.
...
- Output là `VideoSpec` với `scenes[]` đã chia thành chapters, mỗi chapter có `narration` + `visual_type` + `visual_spec` + `beats[]`.
- Mỗi beat = một visual transition trong animation liên tục. `trigger_phrase` = substring chính xác của narration, dùng để sync timing sau TTS.
- HITL: người dùng duyệt/sửa (bao gồm cả beats).

### Stage 3 — Manim codegen (agentic, phần khó nhất)
- Input: các scene `visual_type ∈ {manim, chart}` kèm `beats[]`.
- Codegen sinh **MỘT Scene liên tục** cho tất cả beats — objects persist across beats (giống 3Blue1Brown).
- Code được đánh dấu `# ═══ BEAT {id} ═══` + `self.wait()` ở biên giới beat để compositor có thể tính toán timing.
...
- Chạy SAU voiceover, TRƯỚC composite.
- Input: `word_timestamps` + `beats[].trigger_phrase` cho mỗi scene.
- Logic: substring match (exact → fuzzy Jaccard fallback) trigger_phrase lên running text từ word_timestamps → xác định `start_sec` cho mỗi beat.
- Fallback: nếu match fail → chia đều duration cho các beat (không tệ hơn thiết kế cũ).
- Output: `beats[].start_sec`, `beats[].duration_sec` cho mỗi beat.

### Stage 5 — Composite + subtitle (deterministic, MPT)
- **Beat-aware compositing:** nếu scene có `beats_timed`, chia clip Manim thành N segment (tương ứng N beat), speed-adjust mỗi segment để khớp `beat.duration_sec` từ narration timing.
- Nếu tổng duration gần nhau (±15%), dùng uniform speed change (đơn giản, chất lượng cao).
- Nếu lệch nhiều, per-beat time-stretch với clamp 0.5x–2.0x (ngoài range → freeze/trim).
- Timeline assembler: xếp clip theo `order`, render phụ đề từ `word_timestamps` bằng Pillow.
...
2. **Sync narration ↔ animation** — thứ phân biệt explainer hay với slideshow. 3Blue1Brown làm thủ công. Tự động hóa "animation diễn ra đúng lúc lời thoại nhắc tới" là bài toán lõi khó nhất. Đừng kỳ vọng v1 làm tốt.
   - **Mitigation (beat system):** trigger_phrase matching + per-beat time-stretching giải quyết ~70% trường hợp. Remaining 30% (animation pacing within a beat) cần human review hoặc rate_func tuning.
